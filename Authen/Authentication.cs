...
namespace Authen
{
    ...
    {
        private readonly AppConfigAppSettings _appSettings;
        private readonly string _ldapPath;
        private readonly string _domain;
        private readonly string _reportServer;
        public Authentication(IOptions<AppConfigAppSettings> appSettings)
        {
            _appSettings = appSettings.Value;
            _ldapPath = _appSettings.LdapPath.ToString();
            _domain = _appSettings.Domain;
            _reportServer = _appSettings.ReportServer;

        }
        public bool IsAuthenticated(string _domain, string username, string pwd)
        {
            if (AuthenticateMethod() == "NameOnly") return true;
            if (AuthenticateResult(username, pwd) == "true") return true;
            return false;
        }
        public bool IsAuthenticated(AuthUser authUser)
        {
            if ( Authenticated(authUser) == "true") return true;
            return false;
        }
        public string Authenticated(AuthUser authUser)
        {
            try
            {
                ...
                if (AuthenticateResult(authUser.UserID, authUser.Password) == "true") return "true";
                return "false";
        ...
        public string Authenticated(string domain, string username, string pwd)
        {
            return AuthenticateResult(username, pwd);
        }
        private string AuthenticateResult(string username, string pwd)
        {
            try
            {
        ...
        private string AuthenticateMethod()
        {
            ...
            if (authMethod == "NameOnly")
            {
                ...
                string appServers = _appSettings.AppServers;
